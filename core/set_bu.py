...
class DataSet(dict):
    ...
    def load(self, file_path):
        ...
        self.set_frame(pd.read_csv(os.path.join(copper.config.data, file_path)))

    def restore(self):
        ''' Restores the original version of the DataFrame '''
        self.set_frame(self._oframe)
        self._transformations = []
...
if __name__ == "__main__":
    ...
    ds = copper.read_csv('catalog/training.csv')
    # fnc = lambda x: 12*(2007 - int(str(x)[0:4])) - int(str(x)[4:6]) + 2
    # ds.transform('LASD', fnc)
    # fnc2 = lambda x: math.sqrt(x)
    # ds.transform('RAMN', fnc2)

    # del ds._transformations
    # print(ds.frame)
    # ds['1'] = 1
    a = ds['2']
    print(a)
